...
\title{Consensus-Based Modelling using Distributed Feature Construction\thanks{A short 6-page version of this paper was presented at the 24th International Conference on Inductive Logic Programming, held in conjunction with ECML PKDD, France. Furthermore, a significant part of the work in this paper was done when the first author was an Associate Research Scientist at the Center for Computational Learning Systems (CCLS), Columbia University, NY.}}
 \subtitle{}

\titlerunning{Consensus-Based Modelling} 
...
augmenting any existing features with ILP-discovered relational features can
...
we address this computational difficulty by allowing features to
...
neighbouring nodes share information to update local models. For a category of models
...
datasets that suggest that in relatively short time
...
of models using ILP-based feature discovery. Our approach does not result an optimal answer
...
Figure~\ref{illEx} illustrates this. The algorithm for distributed feature estimation, described in
...
\begin{figure}[htb]
\centerline{\includegraphics[height=0.30\textheight]{ilp_dfe.jpg}}
...
information with its neighbours. Eventually we would like all nodes converge to the same model.}
\label{illEx}
\end{figure}
...
\caption{Consensus-Based Modelling}
\label{alg:fs}
}
\end{algorithm}
...
%\end{equation}
\end{asm}
\noindent Finally, for completion we introduce the notions of \emph{martingales} and the martingale convergence theorem(s) which are required for the proofs in this appendix. 
...
\noindent \textbf{Proposition 1.0. Convergence of the DFE Algorithm } Under the Assumptions 1.0-3.0, there exists some $\eta^0 > 0$, such that if $0 < \eta_i^t < \eta^0$, then:
\begin{enumerate}
...
We use $Model(k,f)$ to denote the model returned the consensus-based
...
\item \textbf{Synthetic. } We use the ``Trains'' problem
    ...
    data for controlled experiments to test principal conjecture
...
\item \textbf{Real. } We report results from experiments conducted using $7$
    ...
    Mutagenesis \cite{King_96}; Carcinogenesis \cite{King_96a}; DssTox \cite{something};
    and 4 datasets arising from the comparison of Alzheimer's drugs
    denoted here as $Amine$, $Choline$, $Scop$ and $Toxic$ \cite{something}.
...
The Prolog compiler used is Yap (version 6.2.0). The programs are executed on
...
difficult to remember concepts with larger that 7 disjuncts.}
        ...
        \item For each node in the network:
            \begin{enumerate}
                \item Set the number of iterations $T$ and initialize the
                    ...
                    $T$ and $\eta_i = \eta$. 
               \item Execute the algorithm described in Section \ref{algo} for $T$ iterations
...
%                the $N$ nodes. Call this the performance of \textbf{Linear+ILP+DFE}
        \item Using a network with a single node:
            \begin{enumerate}
                \item Execute the
                    algorithm described in Section \ref{algo} for $T$ iterations, learning
                    parameter $\eta$, 
...
\item Two sources of sampling variation result with this method. First, variations are possible with
        ...
        from step of drawing features. In effect, we are performing a randomised search for
        good features within a pre-defined feature space. We report averages for $5$ repetitions
        of draws for the target, and $5$ repetitions of the randomised search for a given target.
...
$Centr.$ & 83.3(14.4) & 451.3(206.0)& 92.0(7.1)  & 222.8(139.6) \\
$Distr.$ & 93.4(10.5) & 54.4(15.3)  & 98.7(1.2)  & 41.6(13.7)\\ \hline
\end{tabular}
}}
\end{center}
\begin{center}
...
Model & \multicolumn{2}{|c||}{$Simple$} & \multicolumn{2}{|c|}{$Complex$}\\ \hline
         & Acc.(\%)   & Time (s)       & Acc. (\%)  & Time (s) \\ \cline{2-5}
$Centr.$ & 83.6(20.1) & 107.7(50.2)& 95.3(0.6)  & 410.4(8.8) \\
$Distr.$ & 79.9(10.4) & 39.6(9.5)  & 96.6(0.6)  & 52.9(2.7)\\ \hline
\end{tabular}
}}
\end{center}
\begin{center}
...
(1) There is a significant difference in accuracies between the distributed and
    ...
    we cannot conclude from this that there is either one of the models is better;
...
$Mut188$ & 84.3(2.6) &  76.8(0.0) & 84.6(2.6) & 141.5(9.1)& 2.1(0.2)    & --  \\
$Canc330$ & 67.6(0.5)& 56.8(0.5)  & 50.4(2.8) & 553.2(25.1) & 34.0(5.7) & -- \\
$DssTox$ & 53.8(0.0) &  61.6(1.0) & 64.7(2.0) & 377.9(23.3) & 26.9(1.5) & --\\
$Amine$  & 75.5(0.4) & 76.6(0.8) &  71.4(1.7)         &225.6(19.8) &109.2(44.4)           & -- \\
$Choline$& 72.9(1.3)  &71.2(0.7) &           &264.1(11.6)  &72.4(24.6)           & -- \\
$Scop$   & 67.2(1.5)  & 69.9(0.3) &           &231.5(5.2)&67.7(26.7)           & -- \\
$Toxic$  &  76.3(1.3) &81.4(0.2) &  79.2(1.4)         &253.4(9.3)&50.4(12.5)           & -- \\ \hline
\end{tabular}
}}
\end{center}
...
\begin{description}
    \item[The learning rate $\lambda$.] As will all methods based on stochastic gradient descent,
...
%\end{figure}
    ...
    \item[Convergence Accuracy.] In experiments here (both with synthetic and real data), we have observed that
            ...
            to model that is within some small error bound of the model from a centralised approach with the
...
\begin{enumerate}
 \item Optimally~\cite{l1Reg,cvpr2007,nips2004} or
    heuristically~\cite{JoshiRS08,Amrita12,NageshRCKDB12,ChalamallaNSR08,Specia_09,RamakrishnanJBS07,SpeciaSRN06}
...
\item Optimally~\cite{JawanpuriaNR11,NairSRK12} solve a convex
 ...
 \item Compute all relational features that satisfy some quality criterion by
    ...
    search space~\cite{pei2004,subseqGap2006,bitSpade,antunes2003,han2005,rakesh1995,han2004,gehrke2002,rastogi1999}.
\end{enumerate}
...
conducted in parallel (although this is not done in the paper). As with
most randomised techniques of this kind, not much can be said about the
final model.
    
Perhaps of most interest to the work here is the Sparse
Network Of Winnow (SNoW) classifers described in \cite{Roth_98,CCRR99}.
...
and to deal with very large feature-spaces at once. is also not required within the
...
different support and precision constraints. Finally,
We need to investigate whether certain kinds of network topologies are better than others (currently,
we impose no control, and use a randomly generated network).
Experimentally, we recognise that
